<nav class="sidebar sidebar-offcanvas" id="sidebar">
  <ul class="nav">
    ...
    <?php foreach ($getMenu->result() as $menu): ?>
      <?php  $url_menu = ($menu->type == "controller") ? ADMIN_ROUTE."/$menu->controller" : $menu->controller?>
              ...
              <div class="collapse" id="menus<?=$menu->id_menu?>">
                <ul class="nav flex-column sub-menu">
                  <?php foreach ($getSubMenu->result() as $sub_menu): ?>
                    <?php  $url_sub_menu = ($sub_menu->type == "controller") ? ADMIN_ROUTE."/$sub_menu->controller" : $sub_menu->controller?>
                    <?php if (is_allowed("sidebar_view_".str_replace(" ","_",$sub_menu->menu))): ?>
                      <li class="nav-item">
                        <a class="nav-link <?=$sub_menu->controller == $uri ? 'active':''?>" target="<?=$sub_menu->target?>" href="<?= site_url($url_sub_menu)?>">
        ...
        <?php else: ?>
          <?php if (is_allowed("sidebar_view_".str_replace(" ","_",$menu->menu))): ?>
            <li class="nav-item <?=$menu->controller == $uri ? 'active':''?>">
              <a class="nav-link" target="<?=$menu->target?>" href="<?=site_url($url_menu)?>">
                <i class="<?=$menu->icon?> menu-icon"></i>
                <span class="menu-title"><?=ucwords($menu->menu)?></span>
              </a>
            </li>
